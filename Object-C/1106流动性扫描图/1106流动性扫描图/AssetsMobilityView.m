...
@property (nonatomic, strong) AssetsMobilityDataView *detailDataView;
@end

@implementation AssetsMobilityView
...
-(void)addScrollView
{
    ...
    _detailDataView = [[AssetsMobilityDataView alloc] initWithFrame:CGRectMake(0, 0, 1000, _mHight)];
...
- (void)drawRect:(CGRect)rect {
    ...
    [yPath moveToPoint:CGPointMake(zeroP.x + 0.5, zeroP.y-3*yGap-15)];
    [yPath addLineToPoint:CGPointMake(zeroP.x + 0.5, zeroP.y+3*yGap)];
    ...
    for (int i=0; i<7; i++) {
        ...
        [xPath moveToPoint:CGPointMake(0, zeroP.y + (i-3)*yGap)];
        [xPath addLineToPoint:CGPointMake(_mWidth, zeroP.y + (i-3)*yGap)];
        if (i==3) {
            xPath.lineWidth = 1;
        }
        else{
...
@interface AssetsMobilityDataView()
{
    //居中横坐标
    CGFloat _middleX;
}
...
@implementation AssetsMobilityDataView
-(instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        NSLog(@"%f",xGap);
...
-(void)drawRect:(CGRect)rect
{
    ...
    UIBezierPath *yPath = [UIBezierPath bezierPath];
    for (int i=-5; i<50; i++) {
        [yPath moveToPoint:CGPointMake(_middleX + i*xGap, zeroP.y-3*yGap-15)];
        [yPath addLineToPoint:CGPointMake(_middleX + i*xGap, zeroP.y+3*yGap)];
...
-(void)addCorniceAtPoint:(CGPoint)p height:(CGFloat)h
{
    ...
    UILabel *dateLab = [[UILabel alloc] initWithFrame:CGRectMake(p.x-23, p.y+36*3+3, 46, 14)];
    dateLab.text = @"日期";
