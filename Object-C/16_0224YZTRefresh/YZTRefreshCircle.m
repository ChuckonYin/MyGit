...
//    NSLog(@"++++++%f", angle);
    ...
    if (self.circleState == YZTRefreshCircleStateTurnning && angle>0) {
        return ;
    }
    if (angle<=0.1) {
        self.currentAngle = -M_PI/2;
...
- (void)startTurnningFast{
    ...
    if (!_timer) {
        _timer = [NSTimer timerWithTimeInterval:0.015 target:self selector:@selector(timerAction) userInfo:nil repeats:YES];
