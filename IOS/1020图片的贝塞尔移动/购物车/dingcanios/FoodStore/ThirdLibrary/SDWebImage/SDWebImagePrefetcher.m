...
- (void)reportStatus
{
    ...
    LOG_FORMAT(@"Finished prefetching (%d successful, %d skipped, timeElasped %.2f)", total - _skippedCount, _skippedCount, CFAbsoluteTimeGetCurrent() - _startedTime);
}
...
{
    _finishedCount++;
    LOG_FORMAT(@"Prefetched %d out of %d", _finishedCount, [self.prefetchURLs count]);
...
{
    _finishedCount++;
    LOG_FORMAT(@"Prefetched %d out of %d (Failed)", _finishedCount, [self.prefetchURLs count]);
